...
{
	if (PlayerEXGauge > MaxEXGauge - 1.0f)
	{
		PlayerEXGauge = 0.0f;
...
void Player::PlayerInitialSetting()
{
	...
	if (nullptr != RenderPtr)
	{
		...
		// Interaction & Intro
		RenderPtr->CreateAnimation({ .AnimationName = "ElderKettleInteraction", .SpriteName = "ElderKettleInteraction", .FrameInter = 0.05f, .Loop = false, .ScaleToTexture = true });
...
void Player::PlayerCollisionSetting()
{
	...
	if (nullptr == StandCollisionPtr)
	{
		...
		StandCollisionPtr->SetColType(ColType::AABBBOX2D);
