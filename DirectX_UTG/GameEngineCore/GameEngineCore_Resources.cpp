...
void GameEngineCore::CoreResourcesInit()
{
	...
	// 블랜드 세팅
	{
		...
		Desc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ONE;			// SRC_ALPHA <-> ONE

		// 1번 : 자동으로 알파 부분을 제거해서 출력해주는 것이다. 사용 시 연산이 굉장히 느려서 사용 안함. False
		// 2번 : 블랜드를 여러 개 넣을 것인지 정하는 것. 랜더 타겟이 여러 개일 경우, true로 하면 랜더 타겟 하나 당 블랜드를 하나씩 만들면서 넣어줄 수도 있다.
